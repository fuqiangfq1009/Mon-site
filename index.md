...
        <ul class="nav-links">
                    <li><a href="#" onclick="resetContent()">Accueil</a></li>
...
    
    // Function to restore the content area to the initial "Accueil" state
    function resetContent() {
      // Clone the content of the template and append it to the content area
      const clone = initialContentTemplate.content.cloneNode(true);
      contentArea.innerHTML = ''; // Clear current content
      contentArea.appendChild(clone);
      applyDefaultStyles();
    }

    // Helper function to ensure default styling is applied
    function applyDefaultStyles() {
      contentArea.style.background = "#fff";
      contentArea.style.borderRadius = "8px";
      contentArea.style.padding = "40px 30px";
...
    
    // Function to load content from other pages
    function loadPage(page) {
      contentArea.innerHTML = "<p style='text-align:center; color:#777;'>Chargement...</p>"; // Display loading message
...
        })
        .then(html => {
          contentArea.innerHTML = html; // Insert new content (HTML fragment)
          applyDefaultStyles();
        })
        .catch(err => {
          // FIX 3: Improved error message
          contentArea.innerHTML = "<p style='color:red;'>Impossible de charger le contenu de cette page. Détails : " + err.message + "</p>";
        });
    }
  </script>
</body>
